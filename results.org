...
** OptionAwareNaiveBayesLeftRightMTurk

Same as =OptionAwareNaiveBayes= but with different left and right contexts, trained on ambiguous data. Performed leave one out crossvalidation to obtain results.

|Vote agreement threshold|Classifier trained on *Ambiguous data*|Window size|
|75%|49.5%|1|
|75%|49.5%|3|
|75%|49.5%|10|

The features are actually different for each window size but doesn't seem to affect the results. Same for using different priors.
